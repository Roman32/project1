...
void printWindow(){
	int i;
	for(i = 0; i < 20; i++){
		printf("Window block: %d seq num: %d  ack_flag: %d location: %d\n",i,cliWindow[i].seq_num,cliWindow[i].ack_flag,cliWindow[i].pktStart);
	}
...
int readFromBufferS(char pktBuffer[],int bytesOut, int serverReadIndex){
...
/*
int insertIntoServWindow(int seq_num, int ack_flag, int pktStart, int sizeOfPkt){
	if(numberOfPacketsInWindow  < 20){
		servWindow[windowEndOfPacketBlockPlusOne].seq_num = seq_num;
		servWindow[windowEndOfPacketBlockPlusOne].ack_flag = 0;
		servWindow[windowEndOfPacketBlockPlusOne].pktStart = pktStart;  //byte index of first byte in big buffer
		serviWindow[windowEndOfPacketBlockPlusOne].sizeOfPkt = sizeOfPkt; //use pktStart + sizeOfPkt when obtaining this from buffer
		servWindow[windowEndOfPacketBlockPlusOne].start_time = s_time;
		windowEndOfPacketBlockPlusOne++; //increment window index which is the next "open" spot
		numberOfPacketsInWindow++;
		if(windowEndOfPacketBlockPlusOne == 20){
			windowEndOfPacketBlockPlusOne = 0; //wrap around
...
int removeFromSWindow(int seq_num){
	if(isCWindowFull() == -1){
		perror("tried to remvoe from window when window has no packets");
		return -1;
	}
   ...
   for (i = 0; i < 20; i++){
   	 if(serWindow[i].seq_num == seq_num){
   	 	numberOfPacketsInWindow--;
        	bytesInBuff -= serWindow[i].sizeOfPkt;
   	 	serWindow[i].ack_flag = 1;  // set this to one so we know we can override it
   	 	serWindow[i].pktStart = -1;
   	 	 //prevent some errors i think
   	 	//if this packet was the first packet in our window then we can move the start of packetBlock
   	 	if( i == windowStartOfPacketBlock){
   	 		shift_amount++;
			...
			if(windowStartOfPacketBlock == 20){windowStartOfPacketBlock=0;};
   	 		...
   	 		for(j = i; j < 20; j++){
   	 			//if packet has not been acked
   	 			if(serWindow[j].ack_flag == 0){
   	 				break;
   	 			}else{ //continue moving the start of packet block up;
   	 				windowStartOfPacketBlock++;
                    			bytesInBuff -= serWindow[j].sizeOfPkt;
					if(windowStartOfPacketBlock == 20){windowStartOfPacketBlock=0;};
   	 				shift_amount++;
   	 			}
